...
# ---------------------------------------------------------------------------
# GPTHub routing / memory / guidance functions have been extracted to:
#   - open_webui.utils.gpthub_router
#   - open_webui.utils.gpthub_memory
# Backward-compat aliases for any remaining internal references:
# ---------------------------------------------------------------------------
_extract_requested_model_selection = parse_model_selection
_extract_last_user_prompt = extract_last_user_prompt
_build_voice_ack_text = build_voice_ack_text
_build_manual_mode_guidance_response = build_manual_mode_guidance_response
_is_non_chat_model = is_non_chat_model
_extract_assistant_content_from_response = extract_assistant_content_from_response
...
async def _resolve_effective_model_selection(
    ...
    return await resolve_effective_model_selection(
        request,
        form_data,
        mode,
        requested_model_ids,
        available_models,
        user,
        check_model_access_fn=check_model_access,
        bypass_model_access_control=BYPASS_MODEL_ACCESS_CONTROL,
        bypass_admin_access_control=BYPASS_ADMIN_ACCESS_CONTROL,
        chat_completion_handler_fn=chat_completion_handler,
    )


def _memory_enabled_for_user(request: Request, user, features: dict | None) -> bool:
    return memory_enabled_for_user(
...
async def _run_long_term_memory_write(
    ...
    return await run_long_term_memory_write(
        request,
        user,
        metadata,
        form_data,
        model_id_for_fallback,
        assistant_content=assistant_content,
        chat_completion_handler_fn=chat_completion_handler,
        memories_model=Memories,
        vector_db_client=VECTOR_DB_CLIENT,
        event_emitter_fn=get_event_emitter,
    )


# --- End of extracted function aliases ---
# The following ~1200 lines of function definitions have been removed.
# Original functions are now in gpthub_router.py and gpthub_memory.py.
# ---------------------------------------------------------------------------
...
async def get_manifest_json():
    if app.state.EXTERNAL_PWA_MANIFEST_URL:
        url = app.state.EXTERNAL_PWA_MANIFEST_URL
        from urllib.parse import urlparse

        parsed = urlparse(url)
        if parsed.scheme not in ('http', 'https'):
            raise HTTPException(400, detail='Invalid manifest URL scheme')
        try:
            ...
            raise HTTPException(502, detail='Failed to fetch external manifest')
    else:
        return {
            'name': app.state.WEBUI_NAME,
